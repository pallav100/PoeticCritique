...
const reviewHaikuUsingLLM = async(prompt:string) => {
  const chatCompletion = await openAiInstance.chat.completions.create({
    messages: [{ role: 'user', content: 'Provide a short critique on this haiku - ' +  prompt}],
    model: process.env.OPENAI_MODEL_VERSION || 'gpt-3.5-turbo'
  });
  return chatCompletion?.choices[0]?.message?.content || ""
};

export default reviewHaikuUsingLLM